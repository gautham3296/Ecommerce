...
async function startServer() {
  ...
  app.post("/api/checkout", async (req, res) => {
    try {
      ...
      res.json({ 
        ...
        orderId,
        savedToMySql,
        message: "Order confirmed. Your health soups are being prepared." 
      });
    } catch (e: any) {
  ...
  app.post("/api/recommend", async (req, res) => {
    try {
      ...
      const prompt = `You are an expert botanical health soup guide for "Nalam Brews". 
      ...
      recommend the ONE best blend of our herbal dip soups. Explain why in 2 concise sentences. 
